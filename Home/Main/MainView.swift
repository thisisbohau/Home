...
//            guard let room = updater.status.rooms.first(where: {$0.id == Int(roomId.value ?? "") ?? 0})else{
                ...
                ScrollView{
                    VStack{
                        HStack(alignment: .top){
                            VStack(alignment: .leading){
                                Text("Hello \(String(data: Keychain.load(key: "UserName") ?? Data(), encoding: .utf8) ?? "")")
...
//                            OverviewDivider(title: "Pool", icon: "gridIcon")
//                            OverviewDivider(title: "Mower", icon: "gridIcon")
                        }.padding()
                    }
                }
            }
...
//            .sheet(isPresented: $roomDetected){
//                NavigationView{
//                    RoomView(room: $room)
//                        .navigationTitle(room.id == 0 ? "Updating..." : room.name)
//                }
//
//            }

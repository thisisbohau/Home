...
class IrrigationKit{
    ...
    func pauseSchedule(){
        Task{
            let action = URLQueryItem(name: "action", value: "pauseSchedule")
            let _ = try? await RequestManager().makeActionRequest(requestDirectory: ActionDirectories.irrigation, queries: [action])
        }
...
//        }
        ...
        func startManualIrrigation(zone: irrigationZone, time: Int){
            ...
            let Zone = URLQueryItem(name: "zone", value: String(zone.id))
            let Time = URLQueryItem(name: "time", value: String(time))
                let _ = try? await RequestManager().makeActionRequest(requestDirectory: ActionDirectories.irrigation, queries: [Action, Zone, Time])
            }
...
//        }
    ...
    func modifySchedule(slot: TimeSlot, name: String){
        Task{
            ...
            let _ = try? await RequestManager().makeActionRequest(requestDirectory: ActionDirectories.irrigation, queries: [action, zone, time, minutes])
        }

            ...
            VStack(alignment: .leading){
                ...
                LazyVGrid(columns: columns, alignment: .leading, spacing: 10){
                    ForEach(room.lights){light in
                        DeviceControl(type: light.isHue ? .LightBulbDynamic : .LightBulbMono, status: "", name: light.name, active: true, offStatus: "", selected: scene.lights.contains(where: {$0.id == light.id}), onLongPress: {toggleLight(light: light)}, onTap: {toggleLight(light: light)})
                    }
                    ForEach(room.blinds){blind in
                        DeviceControl(type: .Blind, status: "", name: blind.name, active: true, offStatus: "Closed", selected: scene.blinds.contains(where: {$0.id == blind.id}), onLongPress: {toggleBlind(blind: blind)}, onTap: {toggleBlind(blind: blind)})
                        .id(UUID())
                    }
                    ForEach(room.tempDevices){tado in
                        DeviceControl(type: tado.isAC ? .Cooling : .Heating, status: "", name: tado.name, active: true, offStatus: "Off", selected: scene.tado.contains(where: {$0.id == tado.id}), onLongPress: {toggleTado(tado: tado)}, onTap: {toggleTado(tado: tado)})
                        .id(UUID())
                    }
                }
            }.padding(.bottom)

...
struct AccessoryOverview: View {
    @EnvironmentObject var update: Updater
...
//                HStack{
                    HStack{
//                    Spacer()
//                    StatusItem(active: update.status.laundry.contains(where: {$0.activeCycle}), onDescription: "Running", offDescription: "off", icon: getDeviceIcon(type: .LightBulbMono, state: activeLights != 0, accent: .yellow), onTap: {showLaundry.toggle()})
...
//               AccessoryDetailAvgTemp()
           ...
           .sheet(isPresented: $showGarageControl){
               GarageControl(garage: $update.status.garage)
           }
